...
export function CashierDashboard() {
  ...
  const paymentBreakdown = [
    {
      label: 'M-Pesa',
      value: todaySales.filter((t) => t.paymentMethod === 'mpesa').reduce((s, t) => s + t.total, 0),
      color: COLORS.emerald,
    },
    {
      label: 'Cash',
      value: todaySales.filter((t) => t.paymentMethod === 'cash').reduce((s, t) => s + t.total, 0),
      color: COLORS.amber,
    },
    {
      label: 'Card',
      value: todaySales.filter((t) => t.paymentMethod === 'card').reduce((s, t) => s + t.total, 0),
      color: COLORS.nexora,
    },
  ];
    ...
    <div className="p-6 space-y-5 animate-fade-in">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-surface-400">
            {currentUser.branch} · Cashier · Shift: {formatTime(new Date())}
          </p>
        </div>
        <Button
          variant="primary"
      ...
      <Card className="bg-gradient-to-r from-nexora-50 to-violet-50 dark:from-nexora-500/5 dark:to-violet-500/5 border-nexora-200 dark:border-nexora-500/20">
        <div className="flex items-center justify-between mb-3">
          <div>
            ...
            <p className="text-xs text-surface-500">
              KSh {formatCurrency(todayTotal)} of KSh {formatCurrency(dailyTarget)}
            </p>
          </div>
          <span
            className={cn(
        ...
        <ProgressBar
          value={todayTotal}
          max={dailyTarget}
          color={targetPct >= 100 ? 'emerald' : targetPct >= 50 ? 'nexora' : 'amber'}
          size="md"
        />
      </Card>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          },
          {
            ...
            color: 'text-nexora-600 bg-nexora-50 dark:text-nexora-400 dark:bg-nexora-500/10',
          },
          {
            label: 'Avg Basket',
            ...
            {paymentBreakdown.map((p) => (
              <div key={p.label}>
                ...
                <ProgressBar
                  value={p.value}
                  max={todayTotal || 1}
                  color={p.label === 'M-Pesa' ? 'emerald' : p.label === 'Cash' ? 'amber' : 'nexora'}
                  size="xs"
                  showValue
                />
              </div>
            ...
            <thead className="border-b border-surface-100 dark:border-surface-800">
              <tr>
                {['Receipt', 'Customer', 'Items', 'Payment', 'Total', 'Time'].map((h) => (
                  <th key={h} className="text-left px-4 py-3 font-medium text-surface-400">
                    {h}
                  </th>
                ))}
              </tr>
