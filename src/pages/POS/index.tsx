...
import { Search, ShoppingCart, Plus, Minus, Trash2, UserPlus, Tag, CreditCard, Banknote, Smartphone, ChevronRight, X, Check, Printer, Package } from 'lucide-react';
...
const categories = ['All', 'Electronics', 'FMCG', 'Clothing', 'Hardware', 'Pharmaceuticals', 'Food & Beverage', 'Stationery'];
  ...
  return (
    ...
    >
      <div className="w-full aspect-square bg-surface-50 dark:bg-surface-800 rounded-lg mb-2.5 flex items-center justify-center text-3xl mb-3">
        {product.category === 'Electronics' ? '📱' : product.category === 'FMCG' ? '🛒' : product.category === 'Clothing' ? '👕' : product.category === 'Food & Beverage' ? '🥤' : product.category === 'Pharmaceuticals' ? '💊' : '📦'}
      </div>
      <p className="text-xs font-semibold text-surface-800 dark:text-surface-200 line-clamp-2 leading-tight mb-1">{product.name}</p>
      <p className="text-[10px] text-surface-400 mb-2">{product.sku}</p>
      <div className="flex items-center justify-between">
        <span className="text-sm font-bold text-surface-900 dark:text-surface-50">{formatCurrency(product.price, true)}</span>
        <span className={cn('text-[10px] font-medium', product.stock < product.minStock ? 'text-amber-500' : 'text-surface-400')}>
...
function CartItemRow({ item, onQty, onRemove }: { item: CartItem; onQty: (id: string, d: number) => void; onRemove: (id: string) => void }) {
  return (
    <div className="flex gap-3 py-3 group">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        <div className="flex items-center gap-1 bg-surface-100 dark:bg-surface-800 rounded-lg">
          <button onClick={() => onQty(item.productId, -1)} className="w-6 h-6 flex items-center justify-center text-surface-500 hover:text-surface-800 dark:hover:text-surface-200 rounded-l-lg hover:bg-surface-200 dark:hover:bg-surface-700 transition-colors">
            <Minus size={10} />
          </button>
          <span className="w-7 text-center text-xs font-semibold text-surface-800 dark:text-surface-200">{item.quantity}</span>
          <button onClick={() => onQty(item.productId, 1)} className="w-6 h-6 flex items-center justify-center text-surface-500 hover:text-surface-800 dark:hover:text-surface-200 rounded-r-lg hover:bg-surface-200 dark:hover:bg-surface-700 transition-colors">
            <Plus size={10} />
          </button>
        </div>
        <span className="w-16 text-right text-xs font-bold text-surface-900 dark:text-surface-100">{formatCurrency(item.total, true)}</span>
        <button onClick={() => onRemove(item.productId)} className="w-6 h-6 flex items-center justify-center text-surface-300 hover:text-rose-500 opacity-0 group-hover:opacity-100 transition-all">
          <Trash2 size={12} />
        </button>
      </div>
    </div>
...
function CheckoutModal({ cart, total, customer, onClose, onComplete }: {
  cart: CartItem[]; total: number; customer: Customer | null; onClose: () => void; onComplete: () => void;
}) {
      ...
      <div className="relative bg-white dark:bg-surface-900 rounded-2xl shadow-modal w-full max-w-md overflow-hidden animate-scale-in">
        {done ? (
          <div className="p-10 text-center">
            ...
            <h3 className="text-lg font-bold text-surface-900 dark:text-surface-50 mb-1">Payment Successful!</h3>
            <p className="text-sm text-surface-400 mb-1">Receipt: <span className="font-mono font-medium text-surface-600 dark:text-surface-400">{receiptNo}</span></p>
          ...
          <>
            <div className="flex items-center justify-between px-5 py-4 border-b border-surface-100 dark:border-surface-800">
              ...
              <button onClick={onClose} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-surface-100 dark:hover:bg-surface-800 text-surface-400">
                <X size={16} />
              </button>
            </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="bg-surface-50 dark:bg-surface-800 rounded-xl p-4">
                <div className="flex justify-between text-xs text-surface-500 mb-3">
                  ...
                  {customer && <span className="text-nexora-600 dark:text-nexora-400">👤 {customer.name}</span>}
                </div>
                <div className="text-center">
                  <p className="text-xs text-surface-400 mb-1">Total Amount</p>
                  <p className="text-3xl font-bold text-surface-900 dark:text-surface-50">{formatCurrency(total)}</p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {methods.map((m) => (
                    ...
                    >
                      <span className={method === m.id ? 'text-nexora-600 dark:text-nexora-400' : m.color}>{m.icon}</span>
                      {m.label}
                    </button>
                  ))}
                </div>
              ...
              {method === 'mpesa' && (
                <Input label="M-Pesa Reference (optional)" placeholder="e.g. QH5G7KXXX" value={mpesaRef} onChange={(e) => setMpesaRef(e.target.value)} icon={<Smartphone size={14} />} />
              )}
              {method === 'cash' && (
                <div className="space-y-2">
                  <Input label="Cash Received" placeholder="0.00" type="number" value={cashGiven} onChange={(e) => setCashGiven(e.target.value)} icon={<span className="text-xs font-semibold">KSh</span>} />
                  {parseFloat(cashGiven || '0') >= total && (
                    <div className="flex justify-between items-center px-3 py-2 bg-emerald-50 dark:bg-emerald-500/10 rounded-lg">
                      <span className="text-xs font-medium text-emerald-700 dark:text-emerald-400">Change</span>
                      <span className="text-sm font-bold text-emerald-600 dark:text-emerald-400">{formatCurrency(change)}</span>
                    </div>
                  )}
                </div>
              )}
              {method === 'card' && (
                <div className="p-4 bg-nexora-50 dark:bg-nexora-500/10 rounded-xl text-center">
                  ...
                  <p className="text-xs text-nexora-700 dark:text-nexora-300 font-medium">Insert or tap customer's card</p>
                </div>
              )}
...
export default function POS() {
  ...
  function addToCart(product: Product) {
    setCart((prev) => {
      ...
      return [...prev, { productId: product.id, name: product.name, price: product.price, quantity: 1, discount: 0, total: product.price }];
    });
  }
  ...
  function handleComplete() {
    const rn = `RCP-${new Date().getFullYear()}-${String(Math.floor(Math.random() * 999999)).padStart(6, '0')}`;
    setLastReceipt(rn);
    setCart([]);
  ...
  return (
    <div className="flex h-[calc(100vh-60px)] overflow-hidden">
      {/* Product Panel */}
      <div className="flex-1 flex flex-col min-w-0 border-r border-surface-200 dark:border-surface-800 overflow-hidden">
        {/* Top bar */}
        <div className="px-4 py-3 border-b border-surface-200 dark:border-surface-800 bg-white dark:bg-surface-900 space-y-3">
          <div className="flex gap-2">
            <Input
              ...
              className="flex-1"
            />
            <button ref={barcodeRef} onClick={() => demoToast('Scan', 'Barcode scanner activated', 'info')} className="h-9 px-3 bg-surface-100 dark:bg-surface-800 rounded-lg text-xs font-medium text-surface-500 border border-surface-200 dark:border-surface-700 hover:bg-surface-200 dark:hover:bg-surface-700 transition-colors flex items-center gap-1.5">
              <Package size={14} /> Scan
            </button>
          </div>
          {/* Categories */}
        ...
        {lastReceipt && (
          <div className="px-4 py-2 bg-emerald-50 dark:bg-emerald-500/10 border-t border-emerald-100 dark:border-emerald-500/20 flex items-center justify-between">
            ...
            <button onClick={() => demoToast('Print', 'Receipt sent to printer', 'success')} className="text-xs text-emerald-600 dark:text-emerald-400 font-medium flex items-center gap-1"><Printer size={12} /> Print</button>
          </div>
        )}
      </div>

      {/* Cart Panel */}
      <div className="w-[340px] xl:w-[380px] flex flex-col bg-white dark:bg-surface-900 flex-shrink-0">
        {/* Cart header */}
        <div className="px-4 py-3 border-b border-surface-200 dark:border-surface-800">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-semibold text-surface-900 dark:text-surface-100 flex items-center gap-2">
              ...
              Cart
              {cart.length > 0 && (
                <span className="w-5 h-5 bg-nexora-500 text-white text-[10px] font-bold rounded-full flex items-center justify-center">{cart.length}</span>
              )}
            </h2>
            {cart.length > 0 && (
              <button onClick={() => setCart([])} className="text-xs text-rose-500 hover:text-rose-600 flex items-center gap-1">
          ...
          <div className="relative">
            ...
            >
              {selectedCustomer ? (
                <>
                  ...
                  <span className="flex-1 text-left font-medium truncate">{selectedCustomer.name}</span>
                  <button onClick={(e) => { e.stopPropagation(); setSelectedCustomer(null); }} className="text-surface-400 hover:text-rose-500">
                    <X size={12} />
                  </button>
                </>
              ) : (
            ...
            {showCustomerList && (
              <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-surface-800 border border-surface-200 dark:border-surface-700 rounded-xl shadow-dropdown overflow-hidden z-10">
                <div className="p-2">
                  <Input value={customerSearch} onChange={(e) => setCustomerSearch(e.target.value)} placeholder="Search customers..." icon={<Search size={12} />} />
                </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 divide-y divide-surface-50 dark:divide-surface-800">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center py-12">
              <div className="w-14 h-14 bg-surface-50 dark:bg-surface-800 rounded-2xl flex items-center justify-center text-3xl mb-3">🛒</div>
              <p className="text-sm font-medium text-surface-400">Cart is empty</p>
              <p className="text-xs text-surface-300 dark:text-surface-600 mt-1">Add products from the left panel</p>
            </div>
          ) : (
            cart.map((item) => (
              <CartItemRow key={item.productId} item={item} onQty={updateQty} onRemove={removeItem} />
            ))
          )}
        </div>
          ...
          <div className="px-4 py-4 border-t border-surface-200 dark:border-surface-800 space-y-3">
            ...
            <div className="space-y-1.5 text-xs">
              <div className="flex justify-between text-surface-500">
                <span>Subtotal</span>
                <span className="font-medium text-surface-800 dark:text-surface-200">{formatCurrency(subtotal)}</span>
              </div>
              {discount > 0 && (
              ...
              <div className="flex justify-between text-surface-500">
                <span>VAT (16%)</span>
                <span className="font-medium text-surface-800 dark:text-surface-200">{formatCurrency(taxAmt)}</span>
              </div>
              <Divider />
              <div className="flex justify-between text-base font-bold text-surface-900 dark:text-surface-50 pt-1">
                <span>Total</span>
                <span className="text-nexora-700 dark:text-nexora-400">{formatCurrency(total)}</span>
              </div>
            </div>
      ...
      {showCheckout && (
        <CheckoutModal cart={cart} total={total} customer={selectedCustomer} onClose={() => setShowCheckout(false)} onComplete={handleComplete} />
      )}
    </div>
  );
}
